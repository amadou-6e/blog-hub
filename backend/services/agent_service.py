...
def run_generation(
    *,
    ...
    if result.get("exit_code", 1) != 0:
        ...
        store.complete_job(job_id, error=f"Generation failed: {err}")
        return

    generated_md: str = result.get("stdout", "")
    if not generated_md.strip():
        store.complete_job(job_id, error="Generation produced empty output")
        return

    title = _extract_title(generated_md)
    store.update_article_body(article_id, generated_md, event="Article generated by AI")
    if title != "Untitled Article":
        store.update_article_title(article_id, title)

    if destinations:
        store.set_destinations_pending(article_id, destinations)

    word_count_actual = len(generated_md.split())
    store.complete_job(job_id, result={"wordCount": word_count_actual, "title": title})
