...
def test_connection_save_list_delete_chain(client: TestClient, conn_id: str, secret_name: str):
    ...
    # Insert a disconnected stub so the env-secret fallback is suppressed for this platform.
    # (delete_connection inserts a stub row, overriding env-based auto-detection.)
    store.delete_connection(_UID, conn_id)
    ...
    assert saved_conn["connectedAt"] is not None
    ...
    assert deleted_conn["connectedAt"] is None
