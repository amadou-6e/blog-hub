...
def test_scheduled_medium_sync_uses_connected_browser_profile(store, monkeypatch):
    job = store.create_job(
        store.SEED_USER_ID,
        "sync",
        None,
        payload={"platform": "medium", "scheduled": True},
        queue="sync",
    )
    monkeypatch.setattr(
        store,
        ...
        lambda _user_id, _platform: {
            "status": "connected",
            "skyvern_organization_id": "org-1",
            "skyvern_profile_id": "profile-1",
        },
    )
    monkeypatch.setattr(
        "backend.workers.handlers.runner.medium_browser_articles",
        lambda **_kwargs: {"success": True, "articles": []},
    )
    monkeypatch.setattr(
        "backend.workers.handlers.sync_medium_browser_records",
        lambda _user_id, _retrieval, **_kwargs: {
            "imported": 0,
            "updated": 0,
            "unchanged": 0,
            "failed": 0,
    ...
    worker = DurableWorker(
        store, HANDLERS, worker_id="worker-sync", queues=("sync",), lease_seconds=6,
    )
    assert worker.run_once()
    completed = store.get_job(store.SEED_USER_ID, job["job_id"])
    assert completed["status"] == "completed"
    assert completed["result"]["status"] == "succeeded"
    assert completed["checkpoint"]["platform"] == "medium"
