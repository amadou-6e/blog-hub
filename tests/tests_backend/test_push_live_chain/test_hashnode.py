"""Live push test: Hashnode via the full backend API chain."""
...
def test_push_hashnode_via_backend_api_full_chain(client: TestClient):
    ...
    push_response = client.post(f"/api/articles/{article_id}/push",
    ...
    (_ARTIFACT_DIR / f"backend_hashnode_push_{token}.json").write_text(json.dumps(artifact,
                                                                                  indent=2),
                                                                       encoding="utf-8")
