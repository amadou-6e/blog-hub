...
class TestTitleExtraction:
    ...
    def test_title_extracted_from_h1(self, rendered):
        assert "What Neo4j actually does" in rendered.title

    def test_title_in_head_title_tag(self, rendered):
        assert "What Neo4j actually does" in rendered.html
...
class TestImageNormalisation:
    ...
    def test_remote_images_pass_through_unchanged(self, rendered):
        # The article uses local images rewritten with _IMAGE_BASE_URL as prefix.
        # Confirm at least one rewritten URL appears in the HTML.
        assert "https://cdn.example.com/articles/neo4j" in rendered.html
...
class TestNoRawMarkdown:
    ...
    def test_no_raw_double_asterisk_bold(self, rendered):
        ...
        assert "**Neo4j**" not in rendered.html
        assert "**LlamaIndex**" not in rendered.html
