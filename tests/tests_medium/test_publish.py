...
class TestHtmlDumpTitle:
    ...
    def test_html_dump_title_in_head(self, html_dump):
        assert "Building a Vector DB from Scratch with pgvector" in html_dump
...
class TestHtmlDumpNoPlanningMarkers:
    ...
    def test_tags_value_not_in_dump(self, html_dump):
        # The values after the planning markers should also be gone
        assert "postgres, vector-db, pgvector" not in html_dump
