...
def test_live_overview_articles_feed_includes_real_platform_entries(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert any(
        item["destinations"]["devto"]["status"] != "none"
        or item["destinations"]["hashnode"]["status"] != "none"
        for item in payload["items"]
    )
    assert any(
        (item["destinations"]["devto"]["url"] or "").startswith("https://dev.to/")
        or "hashnode.dev" in (item["destinations"]["hashnode"]["url"] or "")
        for item in payload["items"]
    )
